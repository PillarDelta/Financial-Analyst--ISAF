...
console.log('🔍 METLEN ISAF Analysis - Verbose Test');
console.log('=====================================\n');

try {
    console.log('📊 Sending ISAF analysis request...');
    ...
    console.log('=========================================');
    console.log(result.message?.substring(0, 2000) + '...\n');
    
    // Try different patterns to extract scores
    console.log('🔍 SCORE EXTRACTION ATTEMPTS:');
    console.log('===============================');
    
    const patterns = [
        { name: 'Strategic Score', regex: /Strategic.*?Score[:\s]*([0-9.]+)/i },
        { name: 'PESTEL Score', regex: /PESTEL.*?Score[:\s]*([0-9.]+)/i },
        { name: 'Five Forces Score', regex: /Five.*?Forces.*?Score[:\s]*([0-9.]+)/i },
        { name: 'SWOT Score', regex: /SWOT.*?Score[:\s]*([0-9.]+)/i },
        { name: 'Environmental Score', regex: /Environmental.*?Score[:\s]*([0-9.]+)/i },
        { name: 'Competitive Score', regex: /Competitive.*?Score[:\s]*([0-9.]+)/i },
        { name: 'Influence Score', regex: /Influence.*?Score[:\s]*([0-9.]+)/i },
    ];
    
    patterns.forEach(pattern => {
        const match = result.message?.match(pattern.regex);
        console.log(`${pattern.name}: ${match ? match[1] : 'Not found'}`);
    });
    
    console.log('\n🏷️  EXTRACTION QUALITY:');
    const qualityMatch = result.message?.match(/Extraction.*?quality[:\s]*(\w+)/i);
    console.log(`Quality: ${qualityMatch ? qualityMatch[1] : 'Not found'}`);
    
    console.log('\n📊 ISAF MATHEMATICAL SECTIONS:');
    console.log('================================');
    
    // Look for ISAF mathematical framework sections
    const isafSections = [
        'ISAF V3 Processing',
    ...
    isafSections.forEach(section => {
        const regex = new RegExp(section, 'i');
        const found = result.message?.match(regex);
        console.log(`${section}: ${found ? '✅ Found' : '❌ Not found'}`);
    });
    
    // Save the full response for analysis
    console.log('\n💾 Saving full response for analysis...');
    const fs = await import('fs');
    await fs.promises.writeFile('metlen-full-response.json', JSON.stringify(result, null, 2));
    console.log('📁 Full response saved to: metlen-full-response.json');
